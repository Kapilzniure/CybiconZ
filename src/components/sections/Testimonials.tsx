...
export default function Testimonials() {
  ...
  return (
    <section data-section="testimonials-section" className="py-[100px] relative overflow-hidden" style={{ background: "#060608", borderTop: "1px solid rgba(255,255,255,0.05)", "--float-speed": floatSpeed } as CSSProperties}>
      ...
      {!prefersReduced && <FloatingGeometry variant="ring" color="#EC4899" size={120} opacity={0.10} position={{ bottom: '5%', left: '2%' }} speed={8} />}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-8">
          {items.map((t, i) => (
            <motion.div key={i}
              ...
              custom={i}
              className="rounded-2xl overflow-hidden flex flex-col transition-all duration-250 ease-in-out hover:-translate-y-[6px] relative animate-gpu"
              style={{ background: "#0F0F1C", border: "1px solid rgba(255,255,255,0.07)", transform: "translateZ(0)" }}
            >
